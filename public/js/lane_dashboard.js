...
function ld_get_blocked_reason($exit) {
    ...
    var $visitor_result_strings = [
        ...
        [20,'visitor exit car lock','Car Locked'],
    ];
...
//    $div_inner_html += ld_create_remark_select($last_exit,$matched_entry);
    ...
    if ($last_exit.leave_type==0) {
        $div_inner_html += "<button id=\"button-leave\" onclick=\"ld_on_button_clicked('leave','1')\",type=\"button\" class=\"btn btn-primary btn-sm\">Manually Open Gate</button></div>"
    } else {
        $div_inner_html += "<button id=\"button-leave\" onclick=\"ld_on_button_clicked('leave','1')\",type=\"button\" class=\"btn btn-primary btn-sm\" disabled>Manually Open Gate</button></div>"
    }
    $('#div-last-exit').html($div_inner_html);
...
function ld_req_manually_leave($id,$plate_no,$remark) {
    ...
    $jq_manually_leave = $.ajax({
        ...
        if ($data.code==0) {
            ...
            ld_notify('Manually open gate succeeded','success');
        } else {
...
function ld_on_button_clicked($type, $id) {
    ...
    if ($type == 'leave') {
        swal({
            title             : 'Confirm manually open gate?',
